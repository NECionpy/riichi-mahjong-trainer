...
export function validateHand(hand: Hand): boolean {
  ...
  // 检查是否有重复的牌（超过4张）
  const tileCounts = getTileCounts([...hand.handTiles, hand.winningTile, ...hand.melds.flatMap(m => m.tiles)]);
 ...
 */
export function decomposeHand(tiles: Tile[], existingMelds: Meld[] = []): MeldDecomposition[] {
  ...
  for (const tile of sorted) {
    ...
    if (counts[key] >= 2 && !tried.has(key)) {
      ...
      for (let i = remaining.length - 1; i >= 0 && removed < 2; i--) {
        if (remaining[i].suit === tile.suit && remaining[i].value === tile.value) {
      ...
      // 递归分解剩余的面子，并合并已有鸣牌
      const mentsuList = findMentsu(remaining);
      for (const mentsu of mentsuList) {
        // 只保留恰好凑够 neededMentsu 个面子的结果
...
function findMentsu(tiles: Tile[]): Meld[][] {
  ...
  if (counts[firstKey] >= 3) {
    ...
    for (let i = remaining.length - 1; i >= 0 && removed < 3; i--) {
      if (remaining[i].suit === firstTile.suit && remaining[i].value === firstTile.value) {
        remaining.splice(i, 1);
        removed++;
      }
    }
    ...
    for (const sub of subResults) {
      results.push([{ type: 'pon', tiles: [firstTile, firstTile, firstTile], isOpen: false }, ...sub]);
    }
  }
  ...
  if (firstTile.suit !== 'honor') {
    ...
    if (num <= 7) { // 顺子最大从7开始（789）
      // 查找 num+1 和 num+2
      const next1 = sorted.find(t => t.suit === firstTile.suit && getTileNumber(t) === num + 1);
      const next2 = sorted.find(t => t.suit === firstTile.suit && getTileNumber(t) === num + 2);
      ...
      if (next1 && next2) {
        ...
        const subResults = findMentsu(remaining);
        for (const sub of subResults) {
          results.push([{ type: 'chi', tiles: [firstTile, next1, next2], isOpen: false }, ...sub]);
        }
      }
    }
  }
...
function removeTile(tiles: Tile[], tile: Tile): void {
  const index = tiles.findIndex(t => t.suit === tile.suit && t.value === tile.value);
  if (index !== -1) {
