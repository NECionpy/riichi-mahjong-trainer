...
export interface ScoringResult {
  ...
  yaku: Yaku[]; // 役种列表
  isKeiten: boolean; // 形听（形式听牌：只有宝牌没有役种）
  honba: number; // 本场数
...
function calculateFu(
...
): { fu: number; fuSubtotalBefore: number; breakdown: FuBreakdown } {
  ...
  for (const mentsu of decomposition.mentsu) {
    ...
    const isEffectivelyOpen =
      mentsu.isOpen ||
      (!hand.isTsumo && mentsuContainsWinningTile(mentsu, hand.winningTile));
  ...
  if (fu !== fuSubtotalBefore) {
    items.push({
      label: `向上取整 (${fuSubtotalBefore} → ${fu})`,
      fu: fu,
      type: "rounding",
    });
  }
...
function mentsuContainsWinningTile(mentsu: Meld, winningTile: Tile): boolean {
  return mentsu.tiles.some(
    (t) => t.suit === winningTile.suit && t.value === winningTile.value,
  );
}
...
function isPinfu(
  ...
  if (jantai && isHonorTile(jantai)) {
    const honorValue = jantai.value as HonorType;
    if (
      honorValue === gameContext.round ||
      honorValue === gameContext.playerWind
    )
      return false;
    if (
      honorValue === "white" ||
...
function isSuuankou(hand: Hand, decomposition: MeldDecomposition): boolean {
  ...
  return !(
    jantai.suit === hand.winningTile.suit &&
    jantai.value === hand.winningTile.value
  );
}
...
function isSanshokuDoukou(decomposition: MeldDecomposition): boolean {
  ...
  // 进一步检查是否有相同数字的三色刻子
  for (let num = 1; num <= 9; num++) {
    const man = kotsuList.some(
      (m) => m.tiles[0].suit === "man" && getTileNumber(m.tiles[0]) === num,
    );
    const pin = kotsuList.some(
      (m) => m.tiles[0].suit === "pin" && getTileNumber(m.tiles[0]) === num,
    );
    const sou = kotsuList.some(
      (m) => m.tiles[0].suit === "sou" && getTileNumber(m.tiles[0]) === num,
    );
...
function isShousangen(decomposition: MeldDecomposition): boolean {
  ...
  if (
    jantai &&
    isHonorTile(jantai) &&
    dragonTypes.includes(jantai.value as HonorType)
  ) {
    return true;
  }
  return false;
...
function isSanshokuDoujun(decomposition: MeldDecomposition): boolean {
  ...
  for (let num = 1; num <= 7; num++) {
    const man = chiList.some(
      (m) => m.tiles[0].suit === "man" && getTileNumber(m.tiles[0]) === num,
    );
    const pin = chiList.some(
      (m) => m.tiles[0].suit === "pin" && getTileNumber(m.tiles[0]) === num,
    );
    const sou = chiList.some(
      (m) => m.tiles[0].suit === "sou" && getTileNumber(m.tiles[0]) === num,
    );
    if (man && pin && sou) return true;
  }
...
function isRyanmenWait(hand: Hand, decomposition: MeldDecomposition): boolean {
  ...
  for (const mentsu of decomposition.mentsu) {
    ...
    const idx = tiles.findIndex(
      (t) => t.suit === wt.suit && t.value === wt.value,
    );
    if (idx === -1) continue;
...
function isRyuuiisou(tiles: Tile[]): boolean {
  ...
  for (const tile of tiles) {
    const isGreen = greenTiles.some(
      (g) => g.suit === tile.suit && g.value === tile.value,
    );
...
function calculateBasePoints(fu: number, han: number): number {
  if (han >= 78) return 48000; // 六倍役满
  if (han >= 65) return 40000; // 五倍役满
  if (han >= 52) return 32000; // 四倍役满
  if (han >= 39) return 24000; // 三倍役满
  if (han >= 26) return 16000; // 双倍役满
...
export function getBasePointsName(basePoints: number): string {
  ...
  if (basePoints >= 24000) return "三倍役满";
  if (basePoints >= 16000) return "双倍役满";
  if (basePoints >= 8000) return "役满";
  if (basePoints >= 6000) return "三倍满";
...
export function calculateScore(
  ...
  // 特殊牌型：七对子
  if (isChiitoitsu(allTiles)) {
    const chiitoiDecomp: MeldDecomposition = {
      mentsu: [...hand.melds],
      jantai: [],
    };
  ...
  if (isKokushi(allTiles)) {
    const kokushiDecomp: MeldDecomposition = {
      mentsu: [...hand.melds],
      jantai: [],
    };
  ...
  if (isMenzen && isChuurenpoutou(allTiles)) {
    const chuurenDecomp: MeldDecomposition = {
      mentsu: [...hand.melds],
      jantai: [],
    };
  ...
  if (decompositions.length > 0) {
    for (const decomp of decompositions) {
      ...
      const {
        fu: decompFu,
        fuSubtotalBefore,
        breakdown: decompBreakdown,
      } = calculateFu(hand, decomp, gameContext);
